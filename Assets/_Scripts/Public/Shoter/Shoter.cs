            ...
            case 0: //shoter 가 총알. 충돌인식과 바로 발사 형식이 존재
                ...
                if (rigidbody != null)
                {
                    if (selfShotKey == 0)
                    {
                        ...
                        // 충돌을 감지하는 코루틴을 실행합니다.
                        StartCoroutine(SelfShotDetectObject(rigidbody, dirVector, rayType, rayScale, bulletLayermask));
                    ...
                    else
                    {
                        ...
                        // 충돌을 감지하는 코루틴을 실행합니다.
                        StartCoroutine(SelfShotDetectObject(rigidbody, CollisionTargetDirection, rayType, rayScale, bulletLayermask));
                    }
                }
                else
                {
